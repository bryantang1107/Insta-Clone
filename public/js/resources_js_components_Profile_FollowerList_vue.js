          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                _context.next = 4;
                return axios.get("/profile/".concat(_this.user_id, "/followers?page=") + _this.page);

              case 4:
                response = _context.sent;

                if (((_response$data = response.data) === null || _response$data === void 0 ? void 0 : _response$data.length) > 0) {
                  _this.follower_list = [].concat(_toConsumableArray(_this.follower_list), _toConsumableArray(response.data));
                  _this.filtered_follower_list = _this.follower_list;
                } else {
          ...
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                ...
                _this2.filtered_follower_list = [];
                _this2.page = 1;
                _context2.next = 6;
                return axios.get("/profile/".concat(_this2.user_id, "/followers"));

              case 6:
                response = _context2.sent;
                _this2.isLoading = false;
                _this2.follower_list = response.data;
                _this2.filtered_follower_list = response.data;
                _context2.next = 17;
                break;
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                _context3.prev = 0;
                _context3.next = 3;
                return axios["delete"]("/remove/".concat(_this3.user.user_id), {
                  data: {
                    is_user: _this3.is_user
                  }
                });

              case 3:
                _this3.$store.dispatch("removeFollower");

                _this3.$toast.open({
                  message: "Successfully removed ".concat(_this3.user.user.username, " from follower list!"),
                  type: "success",
                  position: "bottom"
                });
          ...
          while (1) {
            switch (_context4.prev = _context4.next) {
              case 0:
                _context4.prev = 0;
                _context4.next = 3;
                return axios.post("/follow/".concat(_this4.user.user_id), {
                  data: {
                    is_following: _this4.user.is_following,
                    type: "follow"
                  }
                });
              ...
              case 3:
                _this4.$toast.open({
                  message: "You have unfollowed ".concat(_this4.user.user.username, "!"),
                  type: "error",
                  position: "bottom",
                  queue: true
                });

                _this4.$store.dispatch("unfollowUser");
              ...
              case 7:
                ...
                return _context4.abrupt("return", window.location = "/login");

              case 11:
                _this4.$toast.open({
                  message: "Unable to remove ".concat(_this4.user.user.username, " from follower list!"),
